...
async def _load_session(self, token):
    ...
    if session and session.get("expires_at", 0) > time.time(): return session
...
async def _auth_fixed(self, request, guild_id=None):
    ...
    if not session: raise web.HTTPUnauthorized(text="Authentication required.")
    normalized = dict(session); normalized["user_id"] = int(normalized.get("user_id", 0))
    try: normalized["guild_ids"] = [int(x) for x in normalized.get("guild_ids", [])]
    except (TypeError, ValueError): normalized["guild_ids"] = []
    ...
    if guild_id is not None:
        ...
        if gid not in normalized["guild_ids"] and not await _member_can_manage(self, guild, normalized["user_id"]):
...
async def _persist_sessions(self):
    ...
    for session in self.sessions.values():
        try: session["guild_ids"] = [int(x) for x in session.get("guild_ids", [])]
        except (TypeError, ValueError): session["guild_ids"] = []
    stored.update(self.sessions); await self.config.web_sessions.set(stored)


async def _persistent_oauth_callback(self, request):
    try: return await _original_oauth_callback(self, request)
    except Exception:
        await _persist_sessions(self); raise
...
async def _api_guilds(self, request):
    from aiohttp import web
    session = await self._auth(request); user_id = int(session.get("user_id", 0))
    try: oauth_guild_ids = {int(x) for x in session.get("guild_ids", [])}
    except Exception: oauth_guild_ids = set()
    result=[]
    for guild in self.bot.guilds:
        if user_id and oauth_guild_ids and int(guild.id) not in oauth_guild_ids: continue
        if user_id and not oauth_guild_ids and not await _member_can_manage(self, guild, user_id): continue
        result.append({"id":int(guild.id),"name":guild.name,"icon":str(guild.icon.url) if guild.icon else None,"member_count":await _guild_count(self,guild)})
    result.sort(key=lambda x:x["name"].lower()); return web.json_response(result)


async def _api_guild(self, request):
    from aiohttp import web
    gid=int(request.match_info["guild_id"]); await self._auth(request,gid); guild=self.bot.get_guild(gid)
    if guild is None: raise web.HTTPNotFound(text="Guild not found.")
    return web.json_response({"id":int(guild.id),"name":guild.name,"icon":str(guild.icon.url) if guild.icon else None,"member_count":await _guild_count(self,guild),"channels":[{"id":int(c.id),"name":c.name,"type":str(c.type)} for c in guild.channels],"roles":[{"id":int(r.id),"name":r.name,"position":r.position} for r in guild.roles if not r.is_default()]})
...
async def _start_web_fixed(self):
    ...
    app.add_routes([
        web.get("/api/health",self.api_health), web.get("/api/guilds",self.api_guilds),
        web.get("/api/guilds/{guild_id}/events",self.api_events),
        web.get("/api/guilds/{guild_id}/members",self.api_members),
        web.get("/api/guilds/{guild_id}/config",self.api_get_config), web.put("/api/guilds/{guild_id}/config",self.api_put_config),
        web.get("/api/guilds/{guild_id}",self.api_guild),
        web.post("/api/guilds/{guild_id}/announce",self.api_announce),
        web.post("/api/guilds/{guild_id}/moderation/ban",self.api_ban), web.post("/api/guilds/{guild_id}/moderation/kick",self.api_kick), web.post("/api/guilds/{guild_id}/moderation/timeout",self.api_timeout), web.post("/api/guilds/{guild_id}/moderation/delete",self.api_delete),
        web.post("/api/guilds/{guild_id}/members/{action}",self.api_member_action),
