...
function api(path,opts={}){const headers=Object.assign({"Content-Type":"application/json"},opts.headers||{});if(state.token)headers.Authorization=`Bearer ${state.token}`;return fetch(state.api.replace(/\/$/,"")+path,{...opts,headers}).then(async r=>{if(!r.ok)throw new Error((await r.text())||`${r.status} ${r.statusText}`);return r.status===204?{}:r.json()})}
...
async function boot(){
 ...
 try{
  await api("/api/health");$("#apiStatus").textContent="API connected";
  const g=await api("/api/guilds");state.guilds=g;renderGuilds();
  if(g[0]){state.guild=String(g[0].id);$("#guildSelect").value=state.guild;await loadOverview()}
...
async function loadOverview(){
 ...
 const results=await Promise.allSettled([
  api(`/api/guilds/${state.guild}`),
  api(`/api/guilds/${state.guild}/events?limit=500`),
  api(`/api/guilds/${state.guild}/config`)
 ]);
 ...
 if(failed.length)$("#apiStatus").textContent=`API connected · ${failed.length} optional request${failed.length>1?"s":""} failed`;
}
async function loadLogs(){
 if(!state.guild)return;try{state.logs=await api(`/api/guilds/${state.guild}/events?limit=500${$("#logType").value?"&type="+encodeURIComponent($("#logType").value):""}`);renderLogs()}catch(e){toast("Logs failed: "+e.message)}}
...
async function moderate(action){try{const body={user_id:$("#modUser").value,reason:$("#modReason").value};if(action==="timeout")body.minutes=60;await api(`/api/guilds/${state.guild}/moderation/${action}`,{method:"POST",body:JSON.stringify(body)});toast(`${action} completed`);loadLogs();loadOverview()}catch(e){toast("Action failed: "+e.message)}}
