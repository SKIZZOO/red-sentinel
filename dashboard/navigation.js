(()=>{
  ...
  // Capture phase is intentional: app.js stops propagation on nav clicks.
  document.addEventListener('click',e=>{const b=e.target.closest('.nav[data-view]');if(!b)return;const view=b.dataset.view;if(!ROUTES[view])return;const path=routeFor(view);if(location.pathname!==path)history.pushState({sentinelView:view},'',path)},true);
